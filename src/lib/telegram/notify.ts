 ...
 *
 * Sends rich Markdown-formatted alerts to active recipients using their specific bot tokens.
 */
...
/**
 * Escapes all MarkdownV2 reserved characters so Telegram doesn't choke on
 * user-generated strings (emails, company names, snippets, timestamps, etc.).
 *
 * Reserved chars per Telegram docs:
 * _ * [ ] ( ) ~ ` > # + - = | { } . !
 */
...
export async function sendTelegramNotification(
  ...
  chatId: string,
  text: string,
  parseMode: 'MarkdownV2' | 'HTML' | 'plain' = 'plain'
  ...
  try {
    ...
    // Only set parse_mode when explicitly requested — omitting it means plain text
    // and avoids 400 errors from unescaped special characters.
    if (parseMode !== 'plain') {
...
export async function notifyAllRecipients(
  ...
  for (const recipient of recipients) {
    ...
    const res = await sendTelegramNotification(token, recipient.chat_id, message, 'MarkdownV2')
    if (res.success) {
      sentCount++
    } else if (res.error) {

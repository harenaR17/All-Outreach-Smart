...
export function SmartBoxManager({ initialLeads }: Props) {
  ...
  return (
        ...
        {selectedLead ? (
          <>
            ...
            <button
              ...
              className="w-full text-left rounded-2xl bg-zinc-900/40 border border-zinc-800/80 p-4 flex items-center justify-between gap-3 cursor-pointer hover:bg-zinc-900/60 hover:border-zinc-700/80 transition-colors"
            >
    ...
    <div
      ...
      className={`fixed inset-0 z-40 bg-black/80 backdrop-blur-sm transition-opacity duration-200 ${isVariablesOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
        }`}
    />
    <div
      role="dialog"
      aria-label="Lead Fields"
      className={`fixed inset-y-0 right-0 z-50 w-full max-w-sm bg-zinc-950 border-l border-zinc-800 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${isVariablesOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {selectedLead && Object.keys(selectedLead.leadVariables || {}).length > 0 ? (
          Object.entries(selectedLead.leadVariables).map(([key, value]) => (
            <div key={key} className="p-2.5 rounded-lg bg-zinc-900/40 border border-zinc-800/80 min-w-0">

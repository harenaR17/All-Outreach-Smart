...
import { Database, Key, CheckCircle2, AlertCircle, Loader2, ArrowRight, Sparkles, Shield, ExternalLink } from 'lucide-react'
...
export function StepSupabase({ formData, updateFormData, onNext }: StepSupabaseProps) {
  ...
  const handleTest = async () => {
    ...
    if (!formData.managementToken) {
      setErrorMessage('Please provide your Supabase Personal Access Token (sbp_...) to run SQL migrations and deploy edge functions.')
      return
    }
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div>
        ...
        <p className="text-sm text-zinc-400 mt-1">
          Enter your Supabase credentials. All communications are sent directly over HTTPS and work seamlessly across Cloudflare Workers and localhost.
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <p className="text-[11px] text-zinc-500 mt-1">
            Found in Supabase: <span className="text-zinc-400">Project Settings → API → Project API keys → service_role</span>.
          </p>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="text-xs font-medium text-zinc-300 uppercase tracking-wider flex items-center gap-1.5">
              ...
              Supabase Personal Access Token <span className="text-rose-400">*</span>
            </label>
            <a
          ...
          <div className="relative">
            <input
              ...
              value={formData.managementToken}
              onChange={(e) => {
                updateFormData({ managementToken: e.target.value })
                setTestSuccess(null)
              }}
              className="w-full bg-zinc-900/90 border border-zinc-800 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl px-4 py-2.5 text-sm text-zinc-100 placeholder:text-zinc-600 transition-all font-mono"
            />
          ...
          <p className="text-[11px] text-zinc-500 mt-1">
            Used over HTTPS by the setup wizard to execute SQL migrations and deploy edge function secrets.
          </p>
        </div>
      ...
      {testSuccess && (
        <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center gap-3 text-emerald-300 text-xs animate-in fade-in">
          ...
          <div>
            <p className="font-medium">Supabase Project & Management Access Verified!</p>
            <p className="text-emerald-400/80 mt-0.5">REST API, Service Role Key, and Personal Access Token validated over HTTPS.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between pt-2 border-t border-zinc-800/80">
        <button
          type="button"
          onClick={handleTest}
          disabled={testing || !formData.supabaseUrl || !formData.supabaseAnonKey || !formData.supabaseServiceRoleKey || !formData.managementToken}

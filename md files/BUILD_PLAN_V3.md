...
- Leads CRUD includes a genuine external-facing API, not just internal
  dashboard pages — see `leads-api.md` for the auth model (this needed its
  own API-key scheme, since a public API can't rely on the deployment-level
  privacy the rest of the app leans on) and the schema/route additions to
  Phase 2.
...
-- ----------------------------------------------------------------------------
-- api_keys: for the public leads API -- see leads-api.md. Stores a hash, not
-- the raw key, since this is an inbound (verify-only) credential rather than
-- an outbound one the app needs to use itself.
-- ----------------------------------------------------------------------------
...
- [ ] Build the public leads API (auth, endpoints, key management) — see
      `leads-api.md` for the full design and code

**Done when:** importing the same file twice produces zero duplicate `leads` rows and
a "0 new, N already existed" summary the second time, and a request to the public API
with no key or a revoked key correctly gets a 401.
...
- [ ] Build the scheduled edge function, every 30 minutes, same shared-secret check
- [ ] Load every active `campaign_lead` with a non-null `thread_id`
- [ ] Implement classification in order: bounce signals, then auto-reply signals,
      then real reply
- [ ] On bounce: mark the `campaign_lead` bounced and the `lead` globally bounced
- [ ] On auto-reply: log it, leave the sequence untouched
- [ ] On real reply: check the snippet for unsubscribe-style phrasing and set
      `do_not_contact` if matched
- [ ] On real reply (either way): mark the `campaign_lead` replied, clear
      `next_send_at`
- [ ] On a heuristic-`real` message, run the Gemini classification step (see
      `gemini-reply-classification.md` for the full design and code) — treat an
      `out_of_office` result like `auto` instead of stopping the sequence
- [ ] Send a Telegram message to every active row in `telegram_notify_recipients`,
      including the LLM category when available
- [ ] Build the settings page: add/remove/deactivate Telegram recipients (label +
...
instead of incorrectly stopping the sequence.

### Phase 6 — Monitoring & polish
- [ ] Build the per-campaign leads view: status, current step, last-sent time for
      every lead in that campaign
- [ ] Surface failed sends (missing variables, send errors) somewhere actionable
- [ ] Build pause/resume at the campaign level (`campaigns.status`, already respected
      by the sender)

...
- [x] Apply the schema (§7) via the SQL editor or a migration file
- [x] Scaffold the Next.js project (App Router, TypeScript, Tailwind)
- [x] Wire the Supabase client for server-side use only — service-role key stays in
...
- [x] Confirm `supabase functions serve` runs locally without error

**Done when:** a server-rendered page can read and write a test row in `campaigns`.

### Phase 1 — Inbox connections
- [x] Build the "add inbox" form: mailbox address, service account client email,
      private key
- [x] On submit, verify by fetching that mailbox's Gmail profile before saving
- [x] Surface a clear error on failure (delegation not yet authorized for that domain
      is the most likely cause)
- [x] Build the inbox list page: every connected inbox, status, daily limit, spacing
- [x] Make `daily_send_limit` (default 30) and `min_seconds_between_sends`
      (default 180s) directly editable on this page
- [x] Add an active/inactive toggle per inbox

**Done when:** all four quantmnet.xyz mailboxes are connected and show `active`.

### Phase 2 — Leads & import
- [x] Build spreadsheet upload (CSV/XLSX), header row → variable keys
- [x] Implement dedup-on-insert against `leads.email`
- [x] Show an import summary (new vs. already-existed) after each upload
- [x] Let the import flow assign resulting leads to a chosen campaign
- [x] Skip leads already attached to that campaign rather than duplicating
      `campaign_leads` rows
- [x] Build the leads page: email, status, variables, source import, filterable by
      status
- [x] Add a manual "mark do not contact" action per lead
...
- [x] Build campaign CRUD: name, timezone, working days, working hours
- [x] Build the step editor: ordered steps, subject/body templates with
      `{{variable}}` syntax, delay-in-days per step after the first
- [x] Build live preview: pick a real lead, render the active step against their
      actual `variables`
- [x] Visibly flag any `{{token}}` that doesn't resolve for the selected preview lead
- [x] Build inbox assignment: multi-select from connected, active inboxes
- [x] Show each assigned inbox's daily limit / spacing as read-only reference here
...
- [x] Build the scheduled edge function, invoked roughly every minute
- [x] Add the shared-secret check before doing anything
- [x] Implement the per-campaign working-window check (timezone-aware weekday + hour
      range)
- [x] Implement inbox eligibility: `is_active`, sends-since-UTC-midnight
      `< daily_send_limit`, `now() >= next_available_at` (campaign-traffic only, excludes warmup)
- [x] For new-lead (first-touch) candidates: among eligible, campaign-assigned
      inboxes, pick the one with the oldest/null `last_new_lead_sent_at`
- [x] For follow-up candidates: use the lead's existing `email_account_id` only —
      never reassign; skip and retry later if that specific inbox isn't eligible
      right now
- [x] Implement token retrieval (sign JWT, exchange, cache with expiry)
- [x] Implement template rendering with missing-variable detection; on a miss, log a
      failed send and leave `next_send_at` untouched rather than sending it broken
- [x] Implement sending: plain text, `In-Reply-To`/`References` + `threadId` for
      follow-ups
- [x] Label first-touch emails with IMPORTANT and STARRED for easy mailbox distinction from warmups
- [x] Fetch and store the `Message-ID` header of what was just sent, for the next
      follow-up
- [x] Update the lead (`current_step`, `thread_id`, `last_message_id`,
      `next_send_at`, or mark completed)
- [x] Update the inbox (`last_sent_at`; `last_new_lead_sent_at` too, if this was a
...
registered recipient gets a Telegram message within one polling interval.
...
On an auto-reply: log it, leave everything else untouched. On a real reply:
additionally check the snippet for unsubscribe-style phrasing ("unsubscribe," "remove
me," "opt out," "stop emailing," "take me off"); if present, also set the lead
globally to `do_not_contact` with a `status_reason` quoting the trigger. Either way,
mark the `campaign_lead` replied (clearing `next_send_at` — the entire auto-stop
mechanism), log the reply, and send a Telegram message to every active row in
`telegram_notify_recipients`, naming the lead and a short snippet.
